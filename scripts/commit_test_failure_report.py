#!/usr/bin/env python3
"""Persist post-commit test-phase failures for Cursor hooks."""
...
FAILURE_REL = Path(".cursor/hooks/state/commit-test-failure.json")
...
def build_autofix_prompt(failure: dict[str, Any]) -> str:
    label = str(failure.get("label") or "post-commit test step")
    ...
    rerun = (
        ...
        if argv
        else "python scripts/post_commit_gate.py"
    )

    lines = [
        "**Mode: FIX (post-commit test)** — unit tests failed after the last commit (build phase passed).",
        "",
        f"**Failed step:** {label} (exit {exit_code})",
        f"**Working directory:** `{cwd}`",
        f"**Scopes:** {scope_line}",
        "",
        "Read `.cursor/hooks/state/commit-test-failure.json` for full details.",
        "",
        "Fix production code and/or tests with **minimal** diffs. Re-run:",
        f"`{rerun}` (from repo root) or `python scripts/post_commit_gate.py`.",
        "Add tests for new behavior per AGENTS.md.",
        "",
    ...
    body = "\n".join(lines)
    return f"{body}\n\n---\n\n/test-fix FIX\n\nRepair the test failures above. When the full gate passes, report briefly and stop."
