...
describe('displayProxyWs', () => {
  it('maps proxy-ws pathname to display path', () => {
    expect(
      upstreamPathFromProxyWsRequest('/bff/v1/proxy-ws/v1/display/remote-view/ws'),
    ).toBe('/v1/display/remote-view/ws');
  });
  ...
  it('builds wss upstream URL from https display base', () => {
    expect(
      displayWebSocketUrl(
        ...
        '/v1/display/remote-view/ws',
        'ticket=abc',
      ),
    ).toBe('wss://127.0.0.1:8787/v1/display/remote-view/ws?ticket=abc');
  });

  it('reads display id and authorization from query', () => {
    const req = {
      url: '/bff/v1/proxy-ws/v1/display/remote-view/ws?display_id=d1&authorization=Bearer%20tok',
      headers: {},
    } as IncomingMessage;
