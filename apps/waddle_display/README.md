...
- **Curator layers**: **exclusive** replaces the whole program (bootstrap/adoption). **base** drives screen program timing, theme override, ticker visibility, and primary screen/ticker membership. **enhancement** contributes **add** and **remove** ops for screens, ticker tapes, and overlays when its schedule matches. Active membership is the final pool after applying all contributing configurations in **`sort_order`** ascending (ancestor base chain, active base, then enhancements); **higher `sort_order` wins** when the same catalog id is both added and removed. Seed programs include **Weekdays** / **Weekend** enhancements at sort **10**, time-slot bases at **100–130**, and calendar enhancements at **200+**. Schedule rules on each configuration decide when it is active; the display re-resolves membership when curator rows change and on a short poll interval for time-based rules.
- **`GET /v1/curator/active`** (controller **Curators → Active now**): returns which configurations match now (`exclusive`, `base`, `enhancements`), plus **`program_controls`** (`screens_enabled`, `ticker_enabled` from the active primary curator) and **`effective_members`** — merged screen, ticker tape, and overlay catalog ids with operator **`label`** after layered add/remove (same resolution the display rotator uses).
...
- **RSS screen photos** — each **news-family screen** row has **`require_news_photo`** (default **true**; edit on **Screens** in the controller). When true, only RSS rows with a downloaded image are used for that screen’s slide; the **ticker** is unchanged. If a news screen must still run (e.g. **min placements** / data-key minimum) and no image-backed article is available, the curator may place a photo-less row and set **`*_imageMode`** = **`icon`** (per slot for columns/stack) so the UI shows a **newspaper** icon instead of a photo.
