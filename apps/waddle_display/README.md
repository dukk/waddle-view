...
- **Stacking**: when several kinds match, **static image**, **photo slideshow**, **clocks**, **calendar**, and **stock quote** overlays render **below** animated kinds (confetti, hearts, and so on); **bouncing message** tends to sit on top for readability.
...
- **Types**: **`overlay_type`** uses the same slug style as **`screen_type`**. Built-in renderers include **`shape_rain`**, **`birthday_confetti`**, **`bouncing_message`**, **`falling_images`**, **`matrix_rain`**, **`edge_glow`**, **`floating_balloons`**, **`static_image`**, **`photo_slideshow`**, **`digital_clock`**, **`analog_clock`**, **`calendar_month`**, **`calendar_upcoming`**, and **`stock_quote`**. Additional types may be stored and edited over REST; the display ignores unknown types until a renderer exists.
