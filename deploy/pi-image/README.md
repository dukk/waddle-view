...
2. A **release ARM64 Linux bundle** from `flutter build linux --release` (layout includes `waddle_display` next to `lib/` and `data/`). Build this on **ARM64 Linux** (for example GitHub `ubuntu-24.04-arm`, another ARM64 machine, or a Raspberry Pi). Stock Flutter on Windows does **not** produce an ARM64 Linux binary.
...
docker run --rm --privileged \
  -v "$(pwd)/apps/waddle-display/build/linux/arm64/release/bundle:/bundle:ro" \
...
docker run --rm --privileged `
  -v "//c/dev/waddle-view/apps/waddle-display/build/linux/arm64/release/bundle:/bundle:ro" `
