...
3. **Coverage**: CI enforces a **≥ 80% line coverage floor** on `apps/waddle_display/lib/` plus `packages/waddle_shared/lib/` plus `packages/waddle_data_providers/lib/` plus `packages/waddle_plugin_sdk/lib/` per `apps/waddle_display/tool/coverage_check.dart` (excludes **`packages/waddle_plugin_example/`** reference plugin, **`persistence/tables.dart`** declarative schema-only lines, **generated `*.g.dart` Drift**, **`lib/main.dart`** in the display app, and **`lib/display/screen_rotator.dart`** slide-dispatch UI wiring covered indirectly via slide widget tests). The same tool uses a **90% aspirational target**: coverage between **80% and 90%** passes CI but prints a **warning** so contributors keep pushing toward the target. **Drift migration and shared-model unit tests** live under **`packages/waddle_shared/test/`** (`cd packages/waddle_shared` then `flutter test`). **Collector unit tests** also run under **`packages/waddle_data_providers/test/`** (`cd packages/waddle_data_providers` then `dart test`). **Plugin SDK tests** run under **`packages/waddle_plugin_sdk/test/`** (`dart test --coverage=coverage` then `dart run coverage:format_coverage …` before the merged gate). Run from the app directory:
   ...
   - `dart run tool/coverage_check.dart --min=80 --target=90 coverage/lcov.info ../packages/waddle_plugin_sdk/coverage/lcov.info` (defaults match if you omit flags)
