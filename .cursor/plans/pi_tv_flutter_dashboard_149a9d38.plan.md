...
- **CI workflow**: e.g. **`.github/workflows/pi-release.yml`** on **`push` tags `v*`** (and manual `workflow_dispatch`) with **`paths`** including `apps/waddle_view/**` and **`deploy/**`**; upload **`tar.gz`** or **`.zip`** + **SHA256 checksum** to **GitHub Releases** (or artifact store). Version string from **git tag** aligned with **`pubspec.yaml`**.
...
| CI | `.github/workflows/*` — PR workflow (`apps/waddle_view`, analyze/test/coverage); **`pi-release.yml`** (ARM64 release build + artifact upload on tags) |
