...
| GET | `/v1/catalog/category-options` | Required `kind` (`calendar_events`, `jokes`, `trivia`, `photos`, `videos`, `quoterism_quotes`). Returns `{"items":[{"id","label"},...]}` — distinct category ids that have at least one row for that kind (labels from `curator_categories`). Optional `suppressed` when permitted; same integration scope needles as the matching catalog list (`source`, `data_provider`, `integration_type`, etc.). **400** `missing_kind` / `invalid_kind`. |
...
| POST | `/v1/display/catalog/reset-defaults?confirm=yes` | Deletes **all** rows in `screens`, `ticker_tapes`, and `overlays`, clears curator `curator_configuration_members` for entity types `screen` / `ticker` / `overlay`, then re-applies factory seed catalog and built-in program membership. Does not reset themes, integrations, KV tuning, or custom curator configuration rows. **400** `confirm_required` without `confirm=yes`. Requires **`curator.write`**. Response: `{ screens_removed, tickers_removed, overlays_removed, members_removed, screens_seeded, tickers_seeded, overlays_seeded }`. |
