...
const String kTriviaProviderId = 'trivia_openai';
...
class TriviaDataProvider implements IDataProvider {
  ...
  Future<void> collect(DataWriteContext ctx) async {
    ...
    if (token == null || token.isEmpty) {
      ctx.diagnostics.provider('trivia: skip (no API token)');
      return;
    }

    final extra = TriviaProviderExtraConfig.parse(config.configJson);
    if (extra.maxQuestionPerDay < 1) {
      ctx.diagnostics.provider('trivia: skip (maxQuestionPerDay < 1)');
      return;
    }
    ...
    if (eligible.isEmpty) {
      ctx.diagnostics.provider('trivia: no eligible categories');
      return;
    }
    ...
    try {
      ...
      ctx.diagnostics.provider(
        'trivia: POST ${safeHttpUriForLog(uri)} model=${extra.model} '
        'slots=${slots.length}',
      );
      ...
      ctx.diagnostics.providerFail('trivia: collect', e, st);
    }
  }
