...
class BingImageOfDayDataProvider implements IDataProvider {
  ...
  Future<void> collect(DataWriteContext ctx) async {
    ...
    ctx.diagnostics.provider(
      'bing_iotd: collect base=${safeHttpUriForLog(Uri.parse(base))} '
    ...
    try {
      ...
      try {
        ctx.diagnostics.provider(
          'bing_iotd: GET ${safeHttpUriForLog(archiveUri)}',
        );
        ...
        ctx.diagnostics.providerFail('bing_iotd: archive GET', e, st);
        return;
      }

      final images = archiveJson?['images'];
      if (images is! List<dynamic> || images.isEmpty) {
        ctx.diagnostics.provider('bing_iotd: no images in archive');
        return;
      }
      final first = images.first;
      if (first is! Map<String, dynamic>) {
        ctx.diagnostics.provider('bing_iotd: first image not an object');
        return;
      }
      ...
      if (startdate.isEmpty) {
        ctx.diagnostics.provider('bing_iotd: missing startdate');
        return;
      }
      ...
      if (urlbase.isEmpty || !urlbase.startsWith('/')) {
        ctx.diagnostics.provider('bing_iotd: bad urlbase');
        return;
      }
      ...
      try {
        ctx.diagnostics.provider(
          'bing_iotd: GET image ${safeHttpUriForLog(imageUri)}',
        );
        ...
        ctx.diagnostics.providerFail('bing_iotd: image GET', e, st);
        return;
      }
      ...
      ctx.diagnostics.providerFail('bing_iotd: collect', e, st);
    }
  }
