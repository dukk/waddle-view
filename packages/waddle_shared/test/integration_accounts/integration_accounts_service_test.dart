...
void main() {
  ...
  test('integration types share microsoft account type', () {
    ...
    expect(integrationAccountTypesRequiredForIntegration('calendar_google'), [
      kIntegrationAccountTypeGoogle,
    ]);
    expect(integrationAccountTypesRequiredForIntegration('photo_pexels'), [
      kIntegrationAccountTypeApiKeyPexels,
    ]);
  });

  test(
    'syncIntegrationAccountLinks links api key account per integration row',
    () async {
      ...
      final links = await (db.select(
        db.integrationAccountLinks,
      )..where((t) => t.integrationId.equals('pexels_home'))).get();
  ...
  test(
    ...
    () async {
      ...
      await db
          .into(db.integrationAccountLinks)
          .insertOnConflictUpdate(
  ...
  test(
    'createOperatorIntegrationAccount stores api key account and links integrations',
    () async {
      final db = openMemoryDatabase();
      await warmDatabase(db);
      final secrets = InMemorySecretStore();
      ...
      await secrets.write('provider:access_token:$accountId', 'pexels-key');
  ...
  test(
    'createOperatorIntegrationAccount requires facebook oauth client id',
    () async {
      final db = openMemoryDatabase();
      await warmDatabase(db);
      final secrets = InMemorySecretStore();
  ...
  test('createOperatorIntegrationAccount requires oauth client id', () async {
    ...
    )..where((t) => t.id.equals('personal'))).getSingle();
  ...
  test(
    'deleteOperatorIntegrationAccount blocks when linked without confirm',
    () async {
      final db = openMemoryDatabase();
      await warmDatabase(db);
      final secrets = InMemorySecretStore();
      ...
      expect(
        () => deleteOperatorIntegrationAccount(
          db,
          secrets,
          accountId: 'personal',
        ),
  ...
  test(
    'deleteOperatorIntegrationAccount removes oauth account and disables integrations',
    () async {
      final db = openMemoryDatabase();
      await warmDatabase(db);
      final secrets = InMemorySecretStore();
      ...
      expect(
        await (db.select(
          db.integrationAccounts,
        )..where((t) => t.id.equals('personal'))).get(),
        isEmpty,
      );
      final row = await (db.select(
        db.integrations,
      )..where((t) => t.id.equals(integrationId))).getSingle();
  ...
  test(
    'deleteOperatorIntegrationAccount dismisses active OAuth sign-in alert',
    () async {
      final db = openMemoryDatabase();
      await warmDatabase(db);
      final secrets = InMemorySecretStore();
  ...
  test(
    'deleteOperatorIntegrationAccount allows unused account without confirm',
    () async {
      final db = openMemoryDatabase();
      await warmDatabase(db);
      final secrets = InMemorySecretStore();
      ...
      expect(
        await (db.select(
          db.integrationAccounts,
        )..where((t) => t.id.equals('unused_pexels'))).get(),
        isEmpty,
      );
    },
  );
  ...
  test('updateOperatorIntegrationAccountLabel changes display name', () async {
    ...
    await updateOperatorIntegrationAccountLabel(
      db,
      'pexels_home',
      label: 'Pexels home',
    );
    final row = await (db.select(
      db.integrationAccounts,
    )..where((t) => t.id.equals('pexels_home'))).getSingle();
