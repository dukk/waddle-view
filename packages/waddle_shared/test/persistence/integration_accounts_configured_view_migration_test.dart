...
''');
    ...
    final requirements = await db
        .customSelect(
          'SELECT account_type FROM integration_type_required_accounts '
          "WHERE integration_type = 'calendar_outlook'",
        )
        .get();
    expect(requirements.map((r) => r.read<String>('account_type')).toList(), [
      kIntegrationAccountTypeMicrosoftGraph,
    ]);

    var configured = await integrationAccountsConfiguredFromView(
      db,
      'outlook_home',
    );
    ...
    await secrets.write(
      microsoftGraphAccessTokenSecret('ms-user'),
      'token-value',
    );

    configured = await integrationAccountsConfiguredFromView(
      db,
      'outlook_home',
    );
