...
void main() {
  test('resetCatalogToSystemDefaults removes custom catalog and restores factory', () async {
    ...
    await db.into(db.overlays).insert(
    ...
    final customScreen = await (db.select(db.screens)
          ..where((t) => t.id.equals('custom_screen')))
        .getSingleOrNull();
    expect(customScreen, isNull);

    final customTicker = await (db.select(db.tickerTapes)
          ..where((t) => t.id.equals('custom_ticker')))
        .getSingleOrNull();
    expect(customTicker, isNull);

    final customOverlay = await (db.select(db.overlays)
          ..where((t) => t.id.equals('custom_overlay')))
        .getSingleOrNull();
    expect(customOverlay, isNull);

    final tickers = await (db.select(db.tickerTapes)
    ...
    final eveningMembers = await (db.select(db.curatorConfigurationMembers)
          ..where((t) => t.configurationId.equals('evening')))
        .get();
    expect(
      eveningMembers.any((m) => m.entityId == 'custom_screen'),
      isFalse,
    );
